...
class SalesforceBulkTest(unittest.TestCase):
    ...
    def test_raw_query(self):
        ...
        batch_id = bulk.query(job_id,
                              'Select Id,Name,Email from Contact Limit 1000')
    ...
    def test_csv_query(self):
        ...
        batch_id = bulk.query(job_id, 'Select Id,Name,Description '
        ...
        flag = bulk.get_batch_results(job_id, batch_id,
                                      callback=save_results1, parse_csv=True)
        self.assertTrue(flag)
        ...
        batch = len(results) / 3
        self.callback_count = 0
        bulk.get_batch_results(job_id, batch_id, callback=save_results2,
                               parse_csv=True, batch_size=batch)
    ...
    def test_csv_upload(self):
        ...
        for batch_id in batch_ids:
            flag = bulk.get_upload_results(job_id, batch_id,
                                           callback=save_results1)
            self.assertTrue(flag)
            results = self.results
            self.assertTrue(len(results) > 0)
            self.assertTrue(isinstance(results, list))
            self.assertEqual(results[0], UploadResult('Id', 'Success',
                                                      'Created', 'Error'))
        ...
        batch = len(results) / 3
        self.callback_count = 0
        bulk.get_upload_results(job_id, batch_id,
...
if __name__ == '__main__':
    ...
    login = salesforce_oauth_request.login(username=username,
                                           password=password,
                                           cache_session=True)
